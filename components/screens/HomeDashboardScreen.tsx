...
import { dashboardLines } from "@/lib/dashboardMock";
import { getTariffFromFeed } from "@/lib/mockData";
...
export function HomeDashboardScreen() {
  ...
  return (
    <div className="space-y-4 pb-2">
      ...
      <Card>
        <CardContent className="pb-3 pt-4">
          <div className="flex items-center justify-between gap-2">
            ...
            <button type="button" className="text-slate-400" aria-label="Ещё">
          ...
          <div className="mt-2 divide-y divide-slate-100">
            {dashboardLines.map((line) => {
              ...
              return (
                <div key={line.id} className="flex items-center gap-3 py-3 first:pt-1">
        ...
        <CardContent className="pb-4 pt-4">
          ...
          <div className="mt-3 flex gap-2 overflow-x-auto pb-1">
            {aiAvatars.map((label) => (
              <div
                key={label}
                className="flex h-14 w-14 shrink-0 items-center justify-center rounded-full border border-slate-200 bg-gradient-to-br from-white to-slate-50 text-xs font-bold text-slate-700 shadow-softSm"
              >
                {label.slice(0, 2)}
              </div>
            ))}
      ...
      <Card className="border-accent-yellow/40 bg-gradient-to-r from-amber-50/80 to-white">
      ...
      <div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          ...
          <div className="w-[200px] shrink-0 rounded-[22px] bg-accent-dark p-4 text-left text-white shadow-softSm">
      ...
      <div>
        ...
        <div className="flex gap-3 overflow-x-auto pb-1">
          <div className="w-[260px] shrink-0 overflow-hidden rounded-[22px] border border-slate-200 bg-white shadow-softSm">
          ...
          <div className="w-[200px] shrink-0 rounded-[22px] border border-slate-200 bg-slate-50 p-3 text-xs text-slate-600">
      ...
      <TargetMailingModal open={mailingOpen} onClose={() => setMailingOpen(false)} />
    </div>
  );
}
