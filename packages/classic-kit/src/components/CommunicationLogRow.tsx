...
export function CommunicationLogRow({ row }: { row: CommLogRow }) {
  ...
  const content = (
    <div className="flex items-start justify-between gap-3 py-3.5">
      ...
      <div className="flex shrink-0 items-start gap-1.5">
        ...
        <ChevronRight
          className="mt-0.5 h-4 w-4 shrink-0 text-[rgb(var(--border))]"
          aria-hidden
        />
      </div>
    </div>
  ...
  if (row.callId) {
    return (
      <Link href={`/call/${row.callId}/`} className="-mx-1 block rounded-2xl px-1 transition hover:bg-[rgb(var(--surface-2))]">
        {content}
      </Link>
    );
  }
  ...
  return (
    <button
      type="button"
      className="-mx-1 block w-full rounded-2xl px-1 text-left transition hover:bg-[rgb(var(--surface-2))]"
