    ...
    <div className="safe-px flex min-h-dvh flex-col bg-[rgb(var(--bg))] text-[rgb(var(--text))]">
      <header className="flex items-center justify-between border-b border-[rgb(var(--border))]/60 bg-[rgb(var(--bg))]/95 pt-3 pb-3 backdrop-blur">
        <Image
          ...
          width={36}
          height={36}
          className="h-9 w-9 shrink-0 rounded-full shadow-softSm ring-1 ring-[rgb(var(--border))]/60"
        />
        <button
          type="button"
          onClick={() => {
      ...
      <div className="mt-10 flex flex-1 flex-col px-1">
        <h1 className="mt-2 text-[34px] font-medium leading-[1.1] tracking-tight">
          ...
          <span className="mt-1 block lowercase">
            <span className="text-[rgb(var(--muted))]">в </span>
        ...
        <div className="mt-auto pb-[max(32px,env(safe-area-inset-bottom))]">
          <Link
            href={loginHref}
            className="flex w-full items-center justify-center rounded-2xl bg-gradient-to-r from-[#FF6B35] to-[#E8421E] py-4 text-base font-semibold lowercase text-white shadow-soft transition hover:brightness-110 active:translate-y-[1px]"
          >
            войти
          </Link>
        </div>
