...
export function buildNoLiveKeysFallbackResponse(): AssistantPayload {
  return {
    text:
      "ИИ не подключён: в собранном приложении нет ключей API (переменные NEXT_PUBLIC_* подставляются при сборке Next.js). Локально: файл `apps/ai-first/.env.local` и перезапуск `npm run dev`. На хостинге: задайте те же переменные в настройках проекта и выполните новый деплой. Пока доступны быстрые сценарии: счета, звонки, обращения.",
    suggested: ["Счета за март", "Звонки за неделю", "Активные обращения"]
  };
}
