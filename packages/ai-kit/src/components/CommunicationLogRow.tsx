...
export function CommunicationLogRow({ row }: { row: CommLogRow }) {
  ...
  if (row.callId) {
    return (
      <Link href={`/call/${row.callId}/`} className="-mx-1 block rounded-xl px-1 transition hover:bg-slate-50 dark:hover:bg-slate-700/40">
        {content}
      </Link>
    );
  }
  ...
  return (
    <button
      type="button"
      className="-mx-1 block w-full rounded-xl px-1 text-left transition hover:bg-slate-50 dark:hover:bg-slate-700/40"
